...
function resetAutoLoaderBinds(){
	...
	$(".product").unbind("click",addLineItem);
...
function selectCustomer(){
	...
	$.post("/pos/order/customer/dialog/",{"order_id":order_id,"pos_":pos_id,"company":company_,"customer":customer_id},function(data){
		$("body").append('<div class="widget" id="customers_widget"><span class="widget_cls">X</span><span class="widget_error"></span><div class="widget-body"></div>');
...
function voider(){
	...
	var item_code = $("#pos_order.active .line_item.active").attr("code");
	var line_item_voided = $("#pos_order.active .line_item.active.voided");
	voided = 0;
	if(line_item_voided.length < 1)
		voided = 1;

	$.post("/pos/orderline/void/",{"order_id":order_id,"item_code":item_code,"void":voided,"fi":0},function(data){
		$("#pos_content").html(data);
		autoLoader();
	});
}
...
function addLineItem(){
	...
	var tax_rate = parseFloat((company_tax / 100)).toFixed(2);
	...
	var cart_items = $("#pos_order.active #pos_cart_items .line_item");
	...
	item_qty = !isNaN(item_qty) ? item_qty : 0;
	...
	old_ext = (item_qty * item_price);
	var disc_amount = old_ext * item_discount / 100;
	...
	line_item.children("#line_extended").attr("val",new_ext)
	var item_extended = line_item.children("#line_extended").attr("val");
	new_disc_amount = parseFloat(applied_disc.val()) + disc_amount;
	...
	total = cartTotals[3];
	tax_rate = cartTotals[4];

	if(!isNaN(item_qty)){
		$.post("/pos/orderline/",{"cart":0,"tax":item_tax,"order_id":order_id,"item_code":item_code,"name":item_name,"company_id":item_company,"price":item_price,"qty":item_qty,"extended":item_extended,"discount":item_discount,"order_tax":tax,"disc_amount":discount,"sub_total":sub_total,"amount_due":total,"user_id":user_,"tax_rate":tax_rate,"fi":first_item},function(data){
				$("#pos_content").html(data);
				autoLoader();
		});
	}
...
function createOrder(){
	...
	$.post("/pos/order/create/",{"pos_":pos_,"company_id":company_,"customer_id":customer_,"user_id":user_id},function(data){
		$("#pos_content").html(data);
		autoLoader();
	});
}
